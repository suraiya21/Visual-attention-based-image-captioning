...
shuffle = True
# src folders
root_folder = "Data\Images"  # change this
csv_file = "Data\captions.txt"  # change this
...
# model parameters
embed_wts, embed_size = load_embeding("Data\glove.42B.300d.txt", dataset.vocab)  # change path
...
def test_on_img(data, dataloader):
    ...
    features = model.module.EncoderCNN(img[0:1].to(device))
    caps, alphas = model.module.DecoderLSTM.gen_captions(features, vocab=data.vocab)
    caption = ' '.join(caps)
